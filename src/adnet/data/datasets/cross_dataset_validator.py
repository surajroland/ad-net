...
class DatasetStatisticsAnalyzer:
        ...
        for idx in sample_indices:
            try:
                ...
                for instance in sample.instances:
                    ...
                    if hasattr(dataset, 'class_names') and instance.category_id < len(dataset.class_names):
                        ...
                        if self.unified_taxonomy:
                            ...
                            if unified_id is not None:
                                unified_class = self.unified_taxonomy.get_unified_class_name(unified_id)
        ...
        for idx in sample_indices:
            try:
                ...
                if len(seq_info.timestamps) > 1:
                    time_span = (seq_info.timestamps[-1] - seq_info.timestamps[0]) / 1e6  # Convert to seconds
                    if time_span > 0:
        ...
        for idx in sample_indices:
            try:
                ...
                if hasattr(sample.instances[0], 'visibility') and sample.instances:
                    avg_visibility = np.mean([inst.visibility for inst in sample.instances])
    ...
    def _analyze_weather_distribution(self, dataset: BaseDataset) -> Dict[str, Any]:
        ...
        return {
            'weather_distribution': {k: v/total_samples for k, v in weather_counts.items()},
            'time_distribution': {k: v/total_samples for k, v in time_counts.items()},
            'location_distribution': {k: v/total_samples for k, v in location_counts.items()},
    ...
    def _get_analysis_sample_indices(self, dataset: BaseDataset, max_samples: int = 1000) -> List[int]:
...
class DomainGapAnalyzer:
    ...
    def compute_domain_gap(
        ...
        camera_similarity = self._compute_camera_setup_similarity(
            source_stats['camera_characteristics'], target_stats['camera_characteristics']
        )
        
        temporal_similarity = self._compute_temporal_characteristics_similarity(
            source_stats['temporal_characteristics'], target_stats['temporal_characteristics']
        )
        ...
        overall_gap = self._compute_overall_domain_gap(
            class_divergence, spatial_divergence, camera_similarity,
            temporal_similarity, complexity_ratio, weather_divergence
        )
    ...
    def _compute_class_distribution_divergence(
        ...
        try:
            ...
            all_classes = set(source_dist['class_probabilities'].keys()) | \
                         set(target_dist['class_probabilities'].keys())
            ...
            for class_name in sorted(all_classes):
                source_probs.append(source_dist['class_probabilities'].get(class_name, 0))
                target_probs.append(target_dist['class_probabilities'].get(class_name, 0))
    ...
    def _compute_spatial_distribution_divergence(
        ...
        try:
            ...
            for metric in ['distances_from_ego', 'x_positions', 'y_positions', 'object_sizes']:
                if metric in source_spatial and metric in target_spatial:
                    ...
                    mean_diff = abs(source_mean - target_mean) / (abs(source_mean) + abs(target_mean) + 1e-6)
                    std_diff = abs(source_std - target_std) / (source_std + target_std + 1e-6)
    ...
    def _compute_camera_setup_similarity(
        ...
        try:
            ...
            if max(source_num, target_num) > 0:
                num_similarity = min(source_num, target_num) / max(source_num, target_num)
            ...
            if source_focal and target_focal:
                ...
                focal_similarity = 1.0 - abs(source_avg_focal - target_avg_focal) / max(source_avg_focal, target_avg_focal)
            ...
            if source_res and target_res:
                ...
                res_similarity = 1.0 - abs(source_avg_res - target_avg_res) / max(source_avg_res, target_avg_res)
    ...
    def _compute_temporal_characteristics_similarity(
        ...
        try:
            ...
            if 'frame_rates' in source_temporal and 'frame_rates' in target_temporal:
                ...
                fps_similarity = 1.0 - abs(source_fps - target_fps) / max(source_fps, target_fps)
                similarity_score += fps_similarity * 0.6
            
            # Sequence length similarity
            if 'sequence_lengths' in source_temporal and 'sequence_lengths' in target_temporal:
                source_seq = source_temporal['sequence_lengths']['mean']
                target_seq = target_temporal['sequence_lengths']['mean']
                seq_similarity = 1.0 - abs(source_seq - target_seq) / max(source_seq, target_seq)
    ...
    def _compute_scene_complexity_ratio(
        ...
        try:
            source_objects = source_complexity.get('objects_per_frame', {}).get('mean', 1)
            target_objects = target_complexity.get('objects_per_frame', {}).get('mean', 1)
            
            # Ratio closer to 1.0 means similar complexity
            ratio = min(source_objects, target_objects) / max(source_objects, target_objects)
            return ratio
    ...
    def _compute_overall_domain_gap(
        self, class_div: float, spatial_div: float, camera_sim: float,
        temporal_sim: float, complexity_ratio: float, weather_div: float
    ) -> float:
...
class CrossDatasetValidator:
    ...
    def validate_cross_dataset_transfer(
        ...
        for source_dataset in source_datasets:
            for target_dataset in target_datasets:
                ...
                print(f"Analyzing transfer: {source_dataset.__class__.__name__} → {target_dataset.__class__.__name__}")
                ...
                if model_performance_fn:
                    performance_metrics, class_specific_performance = model_performance_fn(
                        source_dataset, target_dataset
                    )
    ...
    def _analyze_failure_modes(
        ...
        if domain_gap.class_distribution_divergence > 0.5:
            failure_analysis['high_risk_classes'] = [
                'vehicle.truck', 'vehicle.construction', 'movable.barrier'
    ...
    def _generate_recommendations(
        ...
        if domain_gap.spatial_distribution_divergence > 0.3:
            ...
            recommendations.append(
                "Apply spatial data augmentation during training"
            )
    ...
    def _save_validation_results(self, results: List[CrossDatasetResults]):
        ...
        # Save summary report
        summary_path = os.path.join(self.output_dir, "cross_dataset_validation_summary.json")
        summary_data = {
    ...
    def _create_domain_gap_heatmap(self, results: List[CrossDatasetResults]):
        ...
        for result in results:
            ...
            gap_matrix[source_idx, target_idx] = result.domain_gap_metrics.overall_domain_gap_score
