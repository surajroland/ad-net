...
from ...interfaces.data.dataset import (
    BaseDataset, Sample, CameraParams, InstanceAnnotation, TemporalSequence,
    DatasetRegistry
)
...
class UnifiedTaxonomy:
    ...
    def __init__(self):
        self.unified_to_id = {name: idx for idx, name in enumerate(self.UNIFIED_CLASSES)}
...
class CoordinateHarmonizer:
    ...
    def harmonize_box_3d(self, dataset_name: str, box_3d: np.ndarray) -> np.ndarray:
        ...
        transformed_box = np.array([
            ...
            transformed_yaw_cos, transformed_yaw_sin,
            velocity
        ])
...
class MultiDatasetLoader(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        print(f"Loaded {len(self.datasets)} datasets with {len(self.sample_indices)} total samples")
    ...
    def _build_sampling_indices(self):
        ...
        if self.sampling_strategy == "sequential":
            ...
            for i, (dataset, weight) in enumerate(zip(self.datasets, normalized_weights)):
...
class CrossDatasetValidator:
    ...
    def evaluate_domain_gap(self, dataset_a: BaseDataset, dataset_b: BaseDataset) -> Dict[str, float]:
        """Evaluate domain gap between two datasets"""
        gap_metrics = {}
        
        # Class distribution similarity
        gap_metrics['class_distribution_kl'] = self._compute_class_distribution_kl(dataset_a, dataset_b)
        
        # Camera setup similarity
        gap_metrics['camera_setup_similarity'] = self._compute_camera_similarity(dataset_a, dataset_b)
        
        # Scene diversity
        gap_metrics['scene_diversity_ratio'] = self._compute_scene_diversity(dataset_a, dataset_b)
        
        return gap_metrics
    
    def _compute_class_distribution_kl(self, dataset_a: BaseDataset, dataset_b: BaseDataset) -> float:
        """Compute KL divergence between class distributions"""
        # Simplified implementation
        return 0.5  # Placeholder
    
    def _compute_camera_similarity(self, dataset_a: BaseDataset, dataset_b: BaseDataset) -> float:
        """Compute camera setup similarity"""
        # Compare number of cameras, resolutions, etc.
        return 0.8  # Placeholder
    
    def _compute_scene_diversity(self, dataset_a: BaseDataset, dataset_b: BaseDataset) -> float:
