...
from ...interfaces.data.dataset import (
    BaseDataset, TemporalDataset, MultiModalDataset,
    Sample, CameraParams, InstanceAnnotation, TemporalSequence,
    register_dataset
)
...
class NuScenesDataset(TemporalDataset, MultiModalDataset):
    ...
    def _load_nuscenes_db(self) -> None:
        ...
        self.db = {}
        for table in ['scene', 'sample', 'sample_data', 'sample_annotation',
        ...
        if os.path.exists(category_path):
            with open(category_path, 'r') as f:
                ...
                self.category_mapping = {cat['token']: cat['name'] for cat in categories}
        ...
        if os.path.exists(attribute_path):
            with open(attribute_path, 'r') as f:
                ...
                self.attribute_mapping = {attr['token']: attr['name'] for attr in attributes}
    ...
    def _get_train_scenes(self) -> List[str]:
        ...
        return [token for token, scene in self.db['scene'].items() 
    ...
    def _build_extrinsic_matrix(self, translation: List[float], rotation: List[float]) -> np.ndarray:
    ...
    def _load_sample_annotations(self, sample_token: str) -> List[InstanceAnnotation]:
        ...
        for ann_token in sample['anns']:
            ...
            box_3d = np.array([
                ...
                np.cos(yaw), np.sin(yaw),         # cos(yaw), sin(yaw)
    ...
    def get_camera_calibration(self, sample_token: str) -> CameraParams:
        ...
        for cam_name in self.CAMERA_NAMES:
            ...
            camera_extrinsics.append(self._build_extrinsic_matrix(
    ...
    def _compute_ego_motion(self, prev_pose: np.ndarray, curr_pose: np.ndarray) -> np.ndarray:
    ...
    def _track_instances(self, sequence: TemporalSequence) -> Dict[int, List[InstanceAnnotation]]:
