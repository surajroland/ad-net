...
class SpatialAugmentation3D(Transform):
    ...
    def _transform_ego_pose(
        ...
        for i in range(len(sample.sequence_info.ego_poses)):
            sample.sequence_info.ego_poses[i] = (
                transform @ sample.sequence_info.ego_poses[i]
            )
...
class TemporalAugmentation(Transform):
    ...
    def _apply_temporal_dropout(self, sample: Sample) -> Sample:
        ...
        sequence_info.frame_indices = [
            sequence_info.frame_indices[i] for i in keep_indices
        ]
    ...
    def _simulate_frame_rate(self, sample: Sample) -> Sample:
        ...
        if current_fps > self.target_fps:
            ...
            # Update sequence info
            sequence_info.frame_indices = [
                sequence_info.frame_indices[i] for i in selected_indices
            ]
...
class CutMix3D(Transform):
    ...
    def _points_in_region(
        ...
        within_bounds = ((points >= region_min) & (points <= region_max)).all(axis=1)
