...
from ...interfaces.data.dataset import BaseDataset, Sample, InstanceAnnotation, TemporalSequence
...
class TemporalSequenceSample:
    ...
    def get_temporal_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'time_span': self.timestamps[-1] - self.timestamps[0] if len(self.timestamps) > 1 else 0,
...
class TemporalSequenceBuilder:
    ...
    def _key_frame_sampling(self, dataset: BaseDataset, center_idx: int) -> List[int]:
        ...
        for i in range(self.sequence_length - 1):
            ...
            if ego_motion_sum >= self.ego_motion_threshold or i == self.sequence_length - 2:
    ...
    def _compute_frame_importance(self, dataset: BaseDataset, frame_idx: int, center_idx: int) -> float:
        ...
        ego_motion = self._compute_ego_motion_between_frames(dataset, frame_idx, center_idx)
        ...
        overlap_ratio = len(frame_instances & center_instances) / max(len(center_instances), 1)
        ...
        complexity_score = len(frame.instances) / 20.0  # Normalize by typical max objects
    ...
    def _track_instances(self, frames: List[Sample]) -> Dict[str, List[InstanceAnnotation]]:
...
class TemporalDataLoader(DataLoader):
    ...
    def __init__(
        ...
        **kwargs
        ...
        temporal_dataset = TemporalDatasetWrapper(
            dataset, sequence_builder, transform
        )
    ...
    def collate_temporal_batch(self, batch: List[TemporalSequenceSample]) -> Dict[str, Any]:
        """
        ...
        for seq_idx, sequence in enumerate(batch):
            ...
            if len(seq_ego_poses) < max_sequence_length:
                padding = np.tile(seq_ego_poses[-1:], (max_sequence_length - len(seq_ego_poses), 1, 1))
...
class TemporalDatasetWrapper(Dataset):
    ...
    def __getitem__(self, index: int) -> TemporalSequenceSample:
        ...
        sequence = self.sequence_builder.build_sequence(self.base_dataset, center_frame_idx)
